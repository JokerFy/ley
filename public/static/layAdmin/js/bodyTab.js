...
layui.define(["element","jquery"],function(exports){
	var element = layui.element,
		...
		Tab = function(){
			this.tabConfig = {
				...
				url : "http://mylay.cn/admin/index/getLeftNav"  //获取菜单json地址
			}
		};
	...
	Tab.prototype.render = function() {
		...
		var navId = this.tabConfig.navId;
        console.log(url);
		$.post(url,data={navId:navId},function(data){
            var test = $(".navBar").html();
			//显示左侧菜单
			if($(".navBar").html() == ''){
				var _this = this;
	...
	Tab.prototype.tabAdd = function(_this){
		...
		if(_this.attr("target") == "_blank"){
			...
			if(that.hasTab(_this.find("cite").text()) == -1 && _this.siblings("dl.layui-nav-child").length == 0){
				...
				//当前窗口内容
				var curmenu = {
				/*	"icon" : _this.find("i.iconfont").attr("data-icon")!=undefined ? _this.find("i.layui-icon").attr("data-icon") : _this.find("i.layui-icon").attr("data-icon"),*/
